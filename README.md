...
```
上面这个首先判断node节点上是否有v-bind这种指令，如果存在的话，我们就发布订阅，怎么发布订阅呢？只需要把当前需要订阅的数据push到watcherTask里面，然后到时候在设置值的时候就可以批量更新了，实现双向数据绑定，也就是下面的操作
```js
that.watcherTask[key].forEach(task => {
    task.update()
})
```
然后push的值是一个Watcher的实例，首先他new的时候会先执行一次，执行的操作就是去把`{{test}}` -> 1，也就是说把我们写好的模板数据更新到模板视图上。
...
完整代码已经放到github上了 -> [MyVue](https://github.com/wclimb/MyVue)
